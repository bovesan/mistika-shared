...
class Cp8Content:    
    ...
    def getMediaDirectory(self,quadrant):
        return "AVCLIP" if quadrant<=self.QUADRANT_4 else "" #mP2 files dont need to be patched
    ...
    def fixXML(self,quadrant,xml):
        res=True
        self.m_node.info("changeP8KmetaData:fixXML:start",u"Patching {}".format(xml)) and res
        try: 
            tree=ET.parse(xml)
            root=tree.getroot()
            ...
            res=self.m_node.critical(u"changeP8KmetaData:fixXML",u"Unable to fix {} file".format(xml)) and res
        return res
        
    def fix(self,quadrant,xml,mxf):
        res=True
        res=self.fixXML(quadrant,xml) and res        
        if res and quadrant<self.QUADRANT_4:
            if not self.m_fixer.fixMXF(mxf,xml):
                res=self.m_node.critical("changeP8KmetaData:fix:fixMXF",u"Unable to patch {} file".format(mxf)) and res
        return res        
...
def isReady(self):
    ...
    if not fixer:
        res=self.critical("changeP8KmetaData:isReady:fixer",u"{} not registered, This node works in OSX only".format(Cp8Content._FIX_NODE_NAME)) and res       
    return res
...
def process(self):
    ...
    for c in self.getConnectorsByName("p8"):
        for p in c.getUniversalPaths():
            ...
            if pack!="Panasonic8k":
                res=self.warning("changeP8KmetaData:isReady:notp8",u"{} is not a Panasonic 8K content. Ignored".format(p.getFilePath())) and res
                continue
            if ubint>=0:
