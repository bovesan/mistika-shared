...
def process(self):
    ...
    if inputList:
        ...
        if res:
            r,remaining,processed=copier.transfer(inputList)
            res=r and res
            if len(processed)==0:
                res=self.info("copyP2Card:process:noProcessed",u"No file copied. Please check available space") and res
            else:
                for p in processed:            
                    res=self.info("copyP2Card:process:processed",u"{} successfully copied".format(p.getFileName())) and res
            if errorIfRemaining>0 and len(remaining)>0:
                if errorIfRemaining==1:
                    res=self.info("copyP2Card:process:remaining",u"Error: Not enough space for all the contents") and res
                elif errorIfRemaining==2:
                    res=self.warning("copyP2Card:process:remaining",u"Error: Not enough space for all the contents") and res
                elif errorIfRemaining==3:
                    res=self.critical("copyP2Card:process:remaining",u"Error: Not enough space for all the contents") and res  
    return res
