...
// Written by Bengt Ove Sannes (c) 2014-2015
// bengtove@bovesan.no
...
void main()
{
	...
	bool maskfirst = bool(param4.b);
    vec4 tpix = vec4(0.0);
    vec4 ipix = vec4(0.0);
	vec2 pos = gl_TexCoord[0].xy;
    vec2 p = pos / res;
	if (maskfirst) {
		vec4 ipix = texture2DRect(image2, gl_TexCoord[0].xy);
	} else {
	...
	if (ipix.a >= 1.0) {
		...
		if (!ignoreleft) {
			...
			while (search.x > 0.0) {
				search += vec2(-1.0, 0.0);
				if (maskfirst) {
					spix = texture2DRect(image2, search);
				} else {
