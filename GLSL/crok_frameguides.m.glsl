...
float adsk_Source_frameratio = float(param11.x) * 1.0;
float adsk_time = float(param11.y) * 1.0;
float adsk_result_pixelratio = float(param11.z) * 1.0;
uniform vec3 param12;
float adsk_result_frameratio = float(param12.x) * 1.0;
uniform vec3 param2;
float ratio = float(param2.y) * 0.01;
float let_blend = float(param2.z) * 0.01;
uniform vec3 param7;
float guide_blend = float(param7.x) * 0.01;
uniform vec3 param8;
float center_blend = float(param8.z) * 0.01;
uniform vec3 param10;
float size = float(param10.x) * 0.01;
uniform vec3 param4;
float l_line_blend = float(param4.z) * 0.01;
float l_ratio = float(param4.y) * 0.01;
float offset = float(param10.z) * 1.0;

uniform vec3 param13;
vec3 tint_action = vec3(param12.y, param12.z, param13.x) * 0.01;
uniform vec3 param14;
vec3 tint_center = vec3(param13.y, param13.z, param14.x) * 0.01;
uniform vec3 param15;
vec3 tint_letterbox = vec3(param14.y, param14.z, param15.x) * 0.01;
uniform vec3 param16;
vec3 tint_l_line = vec3(param15.y, param15.z, param16.x) * 0.01;
uniform vec3 param1;
bool letterbox = bool(param1.x);
uniform vec3 param5;
bool guides = bool(param5.x);
bool center = bool(param7.y);
uniform vec3 param9;
bool counter = bool(param9.x);
uniform vec3 param3;
bool letterbox_line = bool(param3.x);
uniform vec3 param6;
bool relative_guide = bool(param6.y);
bool t_offset = bool(param10.y);
bool relative_lines = bool(param6.z);
vec2 position = vec2(param16.y, param16.z) * 0.01;
...
void main()
{
    ...
    vec3 source = vec3(texture2DRect(image1, uv).rgb);
