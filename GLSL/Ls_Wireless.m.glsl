...
uniform vec3 param6;
float adsk_result_pixelratio = float(param6.x) * 1.0;
vec2 starttrack = vec2(param6.y, param6.z) * 0.01;
uniform vec3 param7;
vec2 endtrack = vec2(param7.x, param7.y) * 0.01;
uniform vec3 param8;
vec2 startoffset = vec2(param7.z, param8.x) * 0.001;
vec2 endoffset = vec2(param8.y, param8.z) * 0.001;
uniform vec3 param4;
float radius = float(param4.z) * 0.01;
uniform vec3 param5;
float restoresize = float(param5.x) * 0.01;
float restoremix = float(param5.y) * 0.01;
uniform vec3 param3;
float curve = float(param3.z) * 0.01;
float hook = float(param4.x) * 0.01;
float angle = float(param5.z) * 0.1;
bool overlay = bool(param4.y);
...
void main() {
    ...
    vec3 frontpix = texture2DRect(image1, res * coords / res).rgb;
    ...
    if(abs(splinesdf(coords, start, bend, end)) > radius) {
        ...
        vec3 color1 = texture2DRect(image1, res * sample1 / res).rgb;
        vec3 color2 = texture2DRect(image1, res * sample2 / res).rgb;
        ...
        for(float i = -restoresize; i < restoresize; i += 0.333333) {
            ...
            blurred += weight * texture2DRect(image1, res * p / res).rgb;
            energy += weight;
        }
        if(energy < 0.001) {
