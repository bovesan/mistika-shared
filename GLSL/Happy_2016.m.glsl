...
uniform vec3 param2;
float NUM_SPARKLES = float(param2.z) * 1.0;
float NUM_SUB_SPARKLES = float(param2.y) * 0.01;

uniform vec3 param1;
float SUB_SPARKLE_CHANCE = float(param1.z) * 0.01;
#define PRIMARY_PARTICLE_COLOR vec3(1., 0.8, 0.5)
#define SECONDARY_PARTICLE_COLOR vec3(1., 0.5, 0.3)
float MOTION_BLUR_AMOUNT = float(param1.x) * 0.01;
float SLOW_MOTION_SPEED = float(param2.x) * 0.01;
float SLOWMO_CYCLE_DURATION = float(param1.y) * 0.01;
uniform vec3 param3;
float NORMAL_MOTION_SPEED = float(param3.y) * 0.01;
...
float Rotate = float(param3.x) * 0.01;
...
vec4 tex3D( in vec3 pos, in vec3 normal, sampler2D sampler ) {
    ...
    return     texture2DRect( sampler, res * pos.yz )*abs(normal.x)+
            texture2DRect( sampler, res * pos.xz )*abs(normal.y)+
            texture2DRect( sampler, res * pos.xy )*abs(normal.z);
}
*/
