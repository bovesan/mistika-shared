...
int maxIterations = int(param20.z);
uniform vec3 param21;
int stepLimit = int(param21.x);
uniform vec3 param9;
int aoIterations = int(param9.y);
//#define antialiasing 0.5            // {"label":"Anti-aliasing", "control":"bool", "default":false, "group_label":"Render quality"}
bool antialiasing = bool(param21.y);
uniform vec3 param22;
bool render_on_black = bool(param22.z);
float aa_strength = float(param21.z) * 0.01;
...
vec3 offset = vec3(param23.x, param23.y, param23.z) * 0.01;
...
vec3 objRotation = vec3(param24.x, param24.y, param24.z) * 0.1;
uniform vec3 param4;
float juliaFactor = float(param4.y) * 0.01;
uniform vec3 param5;
float radiolariaFactor = float(param5.x) * 0.01;
float radiolaria = float(param4.z) * 0.01;

// Camera TAB
float cameraRoll = float(param5.y) * 0.1;
float cameraPitch = float(param5.z) * 0.1;
uniform vec3 param6;
float cameraYaw = float(param6.x) * 0.1;
float cameraFocalLength = float(param6.y) * 0.01;
uniform vec3 param25;
vec3 cameraPosition = vec3(param25.x, param25.y, param25.z) * 0.01;

// Colour TAB
uniform vec3 param10;
int colorIterations = int(param10.y);
uniform vec3 param26;
vec3 color1 = vec3(param26.x, param26.y, param26.z) * 0.01;
uniform vec3 param18;
float color1Intensity = float(param18.y) * 0.01;
uniform vec3 param27;
vec3 color2 = vec3(param27.x, param27.y, param27.z) * 0.01;
float color2Intensity = float(param18.z) * 0.01;
uniform vec3 param28;
vec3 color3 = vec3(param28.x, param28.y, param28.z) * 0.01;
uniform vec3 param19;
float color3Intensity = float(param19.x) * 0.01;
float ambientColor = float(param10.z) * 0.01;
uniform vec3 param11;
float ambientIntensity = float(param11.x) * 0.01;
uniform vec3 param29;
vec3 background1Color = vec3(param29.x, param29.y, param29.z) * 0.01;
uniform vec3 param30;
vec3 background2Color = vec3(param30.x, param30.y, param30.z) * 0.01;

// Shading TAB
uniform vec3 param31;
vec3 light = vec3(param31.x, param31.y, param31.z) * 0.1;
uniform vec3 param32;
vec3 innerGlowColor = vec3(param32.x, param32.y, param32.z) * 0.01;
float innerGlowIntensity = float(param19.y) * 0.01;
uniform vec3 param33;
vec3 outerGlowColor = vec3(param33.x, param33.y, param33.z) * 0.01;
float outerGlowIntensity = float(param19.z) * 0.01;
float fog = float(param9.z) * 0.01;
float fogFalloff = float(param10.x) * 0.01;
float specularity = float(param20.x) * 0.01;
float specularExponent = float(param20.y) * 0.01;
uniform vec3 param8;
float aoIntensity = float(param8.z) * 0.01;
float aoSpread = float(param9.x) * 0.01;
...
bool depthMap = bool(param22.x);
bool super_aa = bool(param22.y);
