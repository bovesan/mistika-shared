...
float Shape = float(param1.y) * 0.01;
float Zoom = float(param2.x) * 0.01;
float Layer = float(param1.x) * 0.01;
uniform vec3 param6;
vec3 Blobs = vec3(param6.x, param6.y, param6.z) * 0.01;
uniform vec3 param7;
vec3 Depth = vec3(param7.x, param7.y, param7.z) * 0.01;

vec2 Resolution = vec2(adsk_result_w * Zoom, adsk_result_h * Zoom);
float Time = adsk_time *-.05 * Speed + Offset;



uniform vec3 param8;
vec2 Camera = vec2(param8.x, param8.y) * 0.01;
