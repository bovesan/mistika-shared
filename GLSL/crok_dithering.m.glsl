...
uniform vec3 param2;
float size = float(param2.y) * 1.0;
uniform vec3 param1;
float scale = float(param1.y) * 0.01;
float d_blend = float(param1.z) * 0.01;
float p_blend = float(param2.z) * 0.01;
uniform vec3 param3;
float c64_blend = float(param3.y) * 0.01;
vec2 res = gl_TexCoord[0].zw;
float adsk_result_w = res.x;
float adsk_result_h = res.y;

bool dithering = bool(param1.x);
bool pixelation = bool(param2.x);
bool c64 = bool(param3.x);
...
void main() { 
    ...
    if ( pixelation )
    {
        ...
        vec3 p_col = texture2DRect(image1, res * pixel).rgb;
