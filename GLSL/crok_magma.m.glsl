...
float adskUID_offset = float(param4.z) * 0.01;
...
uniform vec3 param8;
uniform vec3 param9;
vec3 adskUID_colourWheel1 = vec3(param8.y, param8.z, param9.x) * 0.1;
uniform vec3 param10;
vec3 adskUID_colourWheel2 = vec3(param9.y, param9.z, param10.x) * 0.1;
uniform vec3 param11;
vec3 adskUID_colourWheel3 = vec3(param10.y, param10.z, param11.x) * 0.1;
uniform vec3 param1;
int adskUID_maxSteps = int(param1.x);
float adskUID_minStep = float(param11.y) * 1.0;
uniform vec3 param2;
float adskUID_SphereRadius = float(param2.x) * 0.01;
float adskUID_StepDistanceScale = float(param1.y) * 0.01;
float adskUID_DistThreshold = float(param1.z) * 0.01;
uniform vec3 param5;
int adskUID_aaSteps = int(param5.x);
int adskUID_VolumeSteps = int(param11.z);
int adskUID_blendModes = int(param4.x);
uniform vec3 param12;
float adskUID_Density = float(param12.x) * 1.0;
float adskUID_NoiseFreq = float(param2.z) * 0.01;
float adskUID_NoiseAmp = float(param2.y) * 0.01;
uniform vec3 param13;
vec3 adskUID_NoiseAnim = vec3(param12.y, param12.z, param13.x) * 0.01;
