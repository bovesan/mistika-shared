...
uniform vec3 param3;
int adskUID_aaSteps = int(param3.x);
float adskUID_minStep = float(param1.y) * 0.01;
float adskUID_Detail = float(param1.z) * 0.01;
uniform vec3 param6;
float adskUID_brightness = float(param6.y) * 0.01;
uniform vec3 param5;
float adskUID_contrast = float(param5.z) * 0.01;
float adskUID_saturation = float(param6.x) * 0.01;
uniform vec3 param7;
vec3 adskUID_colourWheel1 = vec3(param6.z, param7.x, param7.y) * 0.1;
//uniform vec3 adskUID_colourWheel2;
uniform vec3 param2;
int adskUID_blendModes = int(param2.z);
int adskUID_noise_version = int(param2.x);
float adskUID_noise_offset = float(param2.y) * 0.01;
float adskUID_gamma = float(param5.y) * 0.01;
float adskUID_scale = float(param7.z) * 1.0;
