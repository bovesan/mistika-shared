...
uniform vec3 param1;
int MaxSteps = int(param1.x);
float MinimumDistance = float(param1.y) * 0.001;
float normalDistance = float(param1.z) * 0.01;

uniform vec3 param2;
int Iterations = int(param2.x);
float Scale = float(param2.y) * 0.01;
float FieldOfView = float(param2.z) * 0.01;
uniform vec3 param3;
float FudgeFactor = float(param3.x) * 0.01;
float NonLinearPerspective = float(param3.y) * 0.01;
bool DebugNonlinearPerspective = bool(param6.y);

uniform vec3 param5;
float Ambient = float(param5.z) * 0.01;
float Diffuse = float(param6.x) * 0.01;
uniform vec3 param7;
vec3 LightColor = vec3(param7.x, param7.y, param7.z) * 0.01;
uniform vec3 param8;
vec3 LightColor2 = vec3(param8.x, param8.y, param8.z) * 0.01;
...
// created by Syntopia in 2013-Apr-13
