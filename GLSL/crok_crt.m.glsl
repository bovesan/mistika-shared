...
uniform vec3 param5;
float offset = float(param5.x) * 1.0;
uniform vec3 param3;
int mask_style = int(param3.z);
float time = adsk_time *.05;

// Emulated input resolution.
vec2 res = vec2(param5.y, param5.z) * 1.0;

// Hardness of scanline.
//  -8.0 = soft
// -16.0 = medium
uniform vec3 param1;
float hardScan = float(param1.z) * 0.01;

// Hardness of pixels in scanline.
// -2.0 = soft
// -4.0 = hard
uniform vec3 param2;
float hardPix = float(param2.x) * 0.01;

// Hardness of short vertical bloom.
//  -1.0 = wide to the point of clipping (bad)
//  -1.5 = wide
//  -4.0 = not very wide at all
float hardBloomScan = float(param3.y) * 0.01;

// Hardness of short horizontal bloom.
//  -0.5 = wide to the point of clipping (bad)
//  -1.0 = wide
//  -2.0 = not very wide at all
float hardBloomPix = float(param3.x) * 0.01;

// Amount of small bloom effect.
//  1.0/1.0 = only bloom
//  1.0/16.0 = what I think is a good amount of small bloom
//  0.0     = no bloom
float bloomAmount = float(param2.z) * 0.01;

// Amount of shadow mask.
float maskDark = float(param4.y) * 0.01;
float maskLight = float(param4.x) * 0.01;

// Display warp.
// 0.0 = none
// 1.0/8.0 = extreme
uniform vec3 param6;
vec2 warp = vec2(param6.x, param6.y) * 1.0;
...
// 2.0  = gaussian
// 3.0  = more square
float shape = float(param2.y) * 0.01;
...
vec3 Fetch(vec2 pos,vec2 off){
  ...
  return ToLinear(texture2DRect(image1,pos.xy,-16.0).rgb);}
...
void main() 
{
    ...
    vec3 col = vec3(0.0);
    if ( mask_style == 1 )
        col = Tri(pos)*Mask1(gl_FragCoord.xy);
    if ( mask_style == 2 )
        col = Tri(pos)*Mask2(gl_FragCoord.xy);
    if ( mask_style == 3 )
        col = Tri(pos)*Mask3(gl_FragCoord.xy);
    if ( mask_style == 4 )
        col = Tri(pos)*Mask4(gl_FragCoord.xy);
