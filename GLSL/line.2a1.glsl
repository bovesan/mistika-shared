...
{
	vec4 pipe, p1, p2;
    vec2 vab, abl, v1, vp, s,v,w, res, co, cs, vr, wr;
   ...
   float l2 = pow(abs(vr.x-wr.x),2.0)+pow(abs(vr.y-wr.y),2.0);
   t = 0.0;
   if (l2 == 0.0) {
   d = distance(co, vr);
   } else {
   ...
   if (t < start) {
       ...
       vec2 proj = vr + t * (wr-vr);
       d = distance(co,proj);
   }
   }
      thx = (athick*(1-t) + bthick*(t)) * th;
    ...
    if (d<thx){
		...
		if (extend < 1.0) {
			...
			pipe = texture2DRect(image1, vec2(t*distance(vr,wr), 0.0));
		}
