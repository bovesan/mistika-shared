...
uniform vec3 param8;
float adsk_time = float(param8.z) * 1.0;
uniform vec3 param9;
float adsk_result_frameratio = float(param9.x) * 1.0;
...
vec2 pos = vec2(param9.y, param9.z) * 0.01;
int pattern_type = int(param1.x);

float time = adsk_time *.05 * speed + offset -1.0;

// polka dots uniforms
float radius = float(param3.z) * 0.01;
uniform vec3 param4;
float dim = float(param4.x) * 0.01;
uniform vec3 param10;
float smooth = float(param10.x) * 1.0;

// zigzag uniforms
uniform vec3 param5;
float zigzag_sharpness = float(param5.z) * 0.01;
float zigzag_style = float(param5.y) * 0.01;

// loading screen uniforms
float loading_inner = float(param4.y) * 0.01;
float loading_outer = float(param4.z) * 0.01;
float loading_sharpness = float(param5.x) * 0.01;

// iching uniforms
uniform vec3 param6;
float iching_amount = float(param6.x) * 0.01;
float iching_detail = float(param6.y) * 0.01;

// truchet uniforms
int truchet_style = int(param6.z);
uniform vec3 param7;
bool truchet_curve = bool(param7.x);
float truchet_frequence = float(param7.y) * 0.01;
float truchet_amplitude = float(param7.z) * 0.01;

// binary uniforms
float binary_seed = float(param8.x) * 0.01;
float binary_move = float(param8.y) * 0.01;
