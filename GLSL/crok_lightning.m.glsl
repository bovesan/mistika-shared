...
uniform vec3 param3;
float rot = float(param3.y) * 0.01;
uniform vec3 param4;
float glow = float(param4.x) * 0.01;
uniform vec3 param1;
float noise = float(param1.z) * 0.01;
float size = float(param3.z) * 0.01;
float bias = float(param1.x) * 0.01;
float smoothness = float(param2.x) * 0.01;
float gain = float(param4.y) * 0.01;
float p1 = float(param5.y) * 1.0;
float p2 = float(param5.z) * 1.0;
uniform vec3 param6;
float moblur_samples = float(param6.x) * 1.0;
float moblur_shutter = float(param6.y) * 1.0;
int branches = int(param1.y);
uniform vec3 param7;
vec2 center = vec2(param6.z, param7.x) * 0.01;

bool out_gamma = bool(param4.z);
bool enbl_moblur = bool(param7.y);
