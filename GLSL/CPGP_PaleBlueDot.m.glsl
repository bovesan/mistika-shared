...
vec4 map( in vec3 pos )
{
    ...
    vec3 tex1    = texture2DRect( image1, res * texC.xy * 2.0 ).xyz * 1.5;
    tex1        = texture2DRect( image1, res * texC.yx * 1.0 + tex1.xy * 0.1 ).xyz;
    
    vec3 tex2    = texture2DRect( image2, res * texC.xy * 2.0 ).xyz * 1.5;
    tex2        = texture2DRect( image2, res * texC.yx * 1.0 + tex1.xy * 0.2 ).xyz;
    
    float mask1    = clamp( tex1.z * -3.0 + 1.2, 0.0, 1.0 );
    float mask2 = clamp( mask1 + tex1.x + ( abs( pos.y * 9.0 ) - 7.0 ), 0.0, 1.0 ) * ( tex2.x + 0.2 );
    
    float tex3    = texture2DRect( image2, res * ( texC.xy + vec2( time * 0.025, sin( time * 0.05 ) * 0.1 ) ) * 3.0 + tex1.xy * 0.1 ).x;
    tex3        = texture2DRect( image1, res * texC.xy * 2.0 - vec2( time * 0.125, cos( time * 0.05 ) * 0.15 ) + tex1.yx * 0.1 ).y - tex3 - clamp( mask1 - 0.75, res * 0.0, res * 1.0 );
