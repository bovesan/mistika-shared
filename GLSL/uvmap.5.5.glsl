...
void main()
{
    ...
    if (param1.r < 1) {
		...
		for(int i=0;i<2;i++)
		{
		  tp += texture2DRect(image2, xy+jitter2[i]*2) * m;
		}
	} else if (param1.r < 3) {
		float m = 1.0 / 3;
		for(int i=0;i<3;i++)
		{
		  tp += texture2DRect(image2, xy+jitter3[i]*2) * m;
		}
	} else if (param1.r < 4) {
		float m = 1.0 / 4;
		for(int i=0;i<4;i++)
		{
		  tp += texture2DRect(image2, xy+jitter4[i]*2) * m;
		}
	} else if (param1.r < 5) {
		float m = 1.0 / 8;
		for(int i=0;i<8;i++)
		{
		  tp += texture2DRect(image2, xy+jitter8[i]*2) * m;
		}
	} else if (param1.r < 6) {
		float m = 1.0 / 15;
		for(int i=0;i<15;i++)
		{
		  tp += texture2DRect(image2, xy+jitter15[i]*2) * m;
		}
	} else if (param1.r < 7) {
		float m = 1.0 / 24;
		for(int i=0;i<24;i++)
		{
		  tp += texture2DRect(image2, xy+jitter24[i]*2) * m;
		}
	} else {
		float m = 1.0 / 66;
		for(int i=0;i<66;i++)
		{
		  tp += texture2DRect(image2, xy+jitter66[i]*2) * m;
		}
	}
