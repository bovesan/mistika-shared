...
uniform vec3 param1;
float fringe = float(param1.z) * 0.005;
float maxblur = float(param1.x) * 0.01;
float gain = float(param1.y) * 0.01;
uniform vec3 param3;
int samples = int(param3.x);
uniform vec3 param2;
int rings = int(param2.z);
float feather = float(param2.y) * 0.01;
float n_amount = float(param3.y) * 0.01;
bool pentagon = bool(param2.x);
float namount = float(param3.z) * 0.01;
uniform vec3 param4;
float bias = float(param4.x) * 0.01;
...
void main() 
{
    ...
    if (autofocus)
    {
        fDepth = linearize(texture2DRect(image2,focus).x);
    }
    ...
    {
        col = texture2DRect(image1, res * gl_TexCoord[0].xy).rgb;
    }
