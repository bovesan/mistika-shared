...
void main()
{
    ...
    int sampleSize = 10, offsetX, offsetY, threadsPerBound = 400;
    float left, right, top, bottom, modulo, limit = 0.0;
    bool doubleBreak;
        ...
        if((pos.x < 1 * threadsPerBound) && (pos.y <= 1.0) && (axis != 2)){
            ...
            for(int x = offsetX; x <= res.x; x+=sampleSize){
                for(int y = offsetY; y <= res.y; y+=sampleSize){
                    if (texture2DRect(image1, vec2(x, y)).a > left) {
                        left = texture2DRect(image1, vec2(x, y)).a;
			if (left >= 1.0) {
