...
void main()
{
    ...
    float t = texture2DRect(image1, res * uv).r;

    vec2 gr = vec2(sin(time+uv.x*0.3),cos(time+uv.y*0.6));
    float r = texture2DRect(image1, res * gr*2.97).r;
    ...
    for ( int i=0; i<32; i++ )
    {
        b += texture2DRect(image1, res *vec2(uv.x*vv,uv.y-r)/(tx.xy*float(i)*(cos(time*r)*0.5+0.5)*3.2)).r;
        c += texture2DRect(image1, res *vec2(uv.x-r,uv.y*vv)/(tx.xy*float(i)*(sin(time*r)*0.5+0.5)*7.0)).r;
    }
    b /= 32;
    c /= 32;
