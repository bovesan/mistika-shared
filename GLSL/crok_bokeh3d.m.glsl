...
uniform vec3 param3;
float adskUID_Speed = float(param3.y) * 0.01;
float adskUID_Offset = float(param3.z) * 0.01;

float adskUID_time = adsk_getTime() * 0.005 * adskUID_Speed + adskUID_Offset;

uniform vec3 param1;
float adskUID_NUM_LIGHTS = float(param1.x) * 0.01;
uniform vec3 param2;
float adskUID__FocalDistance = float(param2.z) * 0.01;
float adskUID__DOF = float(param3.x) * 0.01;
float adskUID__ZOOM = float(param2.y) * 0.01;
uniform vec3 param4;
float adskUID_flicker_speed = float(param4.y) * 0.01;
float adskUID_motion_speed = float(param4.x) * 0.01;
float adskUID_amplitude = float(param1.y) * 0.01;
uniform vec3 param7;
vec3 adskUID_colourWheel1 = vec3(param7.x, param7.y, param7.z) * 0.1;
uniform vec3 param8;
vec3 adskUID_colourWheel2 = vec3(param8.x, param8.y, param8.z) * 0.1;
int adskUID_blendModes = int(param4.z);
float adskUID_height = float(param1.z) * 0.01;
bool adskUID_noise_type = bool(param2.x);
