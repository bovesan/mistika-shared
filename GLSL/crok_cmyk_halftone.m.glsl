...
float pScale = float(param2.x) * 5.0;

uniform vec3 param1;
float pDotsize = float(param1.z) * 0.05;
float pMax = float(param1.y) * 0.1;
float pMin = float(param1.x) * 0.05;
...
vec4 halftone(in vec2 fc,in mat2 m)
{
    ...
    vec4 c = rgb2cmyki(texture2DRect(image1,px2uv(smp+ORIGIN)));
    return c+s;
}
