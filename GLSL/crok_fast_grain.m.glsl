...
uniform vec3 param1;
float grain_size = float(param1.z) * 0.005;
float grainamount = float(param1.x) * 0.001;
bool colored = bool(param1.y);
float coloramount = float(param2.x) * 0.005;
float lumamount = float(param2.y) * 0.01;
uniform vec3 param3;
float scratch_opacity = float(param3.x) * 1.0;
float dust_opacity = float(param3.y) * 1.0;
float flicker_opacity = float(param3.z) * 1.0;
uniform vec3 param4;
float flicker_frequency = float(param4.x) * 1.0;
...
void main() 
{
    ...
    vec3 col = texture2DRect(image1, res * texCoord).rgb;
