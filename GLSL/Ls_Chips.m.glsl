...
uniform vec3 param8;
float adsk_result_frameratio = float(param8.y) * 1.0;
uniform sampler2DRect image2;
uniform sampler2DRect image1;
uniform vec3 param9;
vec2 chip1pos = vec2(param8.z, param9.x) * 0.001;
vec2 chip2pos = vec2(param9.y, param9.z) * 0.001;
uniform vec3 param10;
vec2 chip3pos = vec2(param10.x, param10.y) * 0.001;
uniform vec3 param11;
vec3 chip1col = vec3(param10.z, param11.x, param11.y) * 0.01;
uniform vec3 param12;
vec3 chip2col = vec3(param11.z, param12.x, param12.y) * 0.01;
uniform vec3 param13;
vec3 chip3col = vec3(param12.z, param13.x, param13.y) * 0.01;
uniform vec3 param2;
bool chip1nearest = bool(param2.z);
uniform vec3 param4;
bool chip2nearest = bool(param4.z);
uniform vec3 param6;
bool chip3nearest = bool(param6.z);
uniform vec3 param7;
float blockup = float(param7.x) * 1.0;
bool showblocky = bool(param7.y);
bool voronoi = bool(param8.x);
bool palette = bool(param7.z);
...
void getstr(int row, int col) {
  ...
  for(int i = 0; i < 20; i++) {
    ...
    uv /= 1024.0;
    float texel = texture2DRect(image1, res * uv).g;
    str[i] = int(texel);
    if(texel == 0.0) break;
  }
}
...
float print(vec2 where, float size) {
  ...
  float sdf = texture2DRect(image1, res * fontuv).r;
...
vec3 getcol(int row, int col) {
  ...
  uv += 0.5; // Sample texel centres
  uv /= 1024.0;
  vec3 texel = texture2DRect(image1, res * uv).gba;
  return texel;
}
...
vec3 getpalette(vec2 uv) {
  ...
  return texture2DRect(image1, res * st).gba;
}
...
void main() {
  ...
  if(palette) {
    ...
    bg = texture2DLod(image2, xy, lod).rgb;
  }
  ...
  for(int i = 0; i < 3; i++) {
    ...
    if(palette) {
      ...
      pickcol = texture2DLod(image2, pos, blockup + 0.4).rgb;
    }
