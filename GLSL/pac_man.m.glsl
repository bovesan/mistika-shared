...
float pRight = float(param1.z) * 1.0;
uniform vec3 param2;
float pLeft = float(param2.x) * 1.0;
float pUp = float(param2.y) * 1.0;
float pDown = float(param2.z) * 1.0;
float speed = float(param1.x) * 0.01;

vec2 iResolution = vec2(adsk_result_w, adsk_result_h);
vec4 iMouse = vec4(pRight, pLeft, pUp, pDown);
float iGlobalTime = adsk_time*.05;


// Pac man
// created by tsone in 2013-May-25
...
float smootherstep01(in float x) {
    return dot(
        vec3(10.0,-15.0,6.0),
        vec3(pow(x, 3.0),pow(x, 4.0),pow(x, 5.0))
    );
}
