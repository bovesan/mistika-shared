...
float velocity = float(param1.y) * 0.01;
uniform vec3 param2;
vec2 distortion = vec2(param2.y, param2.z) * 0.01;
uniform vec3 param3;
...
vec3 sample(const int x, const int y, vec2 delta)
{
    ...
    uv = uv + delta;
    return texture2DRect(image1, res * uv).xyz;
}
 ...
 void main (void) 
 {
    ...
    uv = uv + delta; 
     ...
     for (int j = -radius; j <= 0; ++j)  {
         for (int i = -radius; i <= 0; ++i)  {
             c = texture2DRect(image1, res * uv + vec2(i,j) * src_size).rgb;
             m0 += c;
             s0 += c * c;
         }
     }
    ...
    vec3 col3 = texture2DRect(image1, res * gl_FragCoord.xy / iResolution.xy + col2.xy * 0.05 ).xyz;
