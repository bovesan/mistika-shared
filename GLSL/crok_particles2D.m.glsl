...
uniform vec3 param3;
float adsk_result_frameratio = float(param3.y) * 1.0;
...
uniform vec3 param1;
float baseRadius = float(param1.x) * 0.01;
float colorVariation = float(param3.x) * 0.01;
float brightnessVariation = float(param2.z) * 0.01;
vec3 backgroundColor=vec3(0.0,0.0,0.0);
float variation = float(param1.y) * 0.01;
float size = float(param1.z) * 0.01;
uniform vec3 param4;
float noise_multiplier = float(param4.x) * 1.0;
