...
uniform vec3 param2;
float mixx = float(param2.x) * 0.01;
...
//   0.00 - completely off
uniform vec3 param1;
float fxaaQualitySubpix = float(param1.y) * 0.01;
...
float fxaaQualityEdgeThreshold = float(param1.z) * 0.01;

// Trims the algorithm from processing darks.
//   0.0833 - upper limit (default, the start of visible unfiltered edges)
//   0.0625 - high quality (faster)
//   0.0312 - visible limit (slower)
float fxaaQualityEdgeThresholdMin = float(param1.x) * 0.001;
...
void main() {
  ...
  vec4 orig = texture2DRect(image1, res * coords);
  vec4 aa = FxaaPixelShader(coords, vec4(0.0), image1, image1, image1,
