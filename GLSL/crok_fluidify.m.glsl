...
uniform vec3 param1;
float detail = float(param1.x) * 0.001;
vec2 iResolution = vec2(adsk_result_w, adsk_result_h);
float iGlobalTime = adsk_time *.05;
#define iFrame adsk_time
#define angRnd 0.0

int RotNum = int(param1.y);
float posRnd = float(param1.z) * 0.001;
...
float getRot(vec2 uv, float sc)
{
    ...
    for(int i=0;i<RotNum;i++)
    {
        ...
        vec2 v = texture2DRect(image2, res *fract(uv+p2)).xy-vec2(0.5);
...
void main()
{
    ...
    gl_FragColor = texture2DRect(image2, res *fract(uv+v*3.0 /iResolution.x));

    // add a little "motor" in the center
    //gl_FragColor.xy += (0.01*scr.xy / (dot(scr,scr)/0.1+0.3));

    if(iFrame<=1 ) gl_FragColor = texture2DRect(image1, res *uv);
}
