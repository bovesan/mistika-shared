...
uniform vec3 param1;
float Rotation = float(param1.x) * 10.0;
float pLeft = float(param2.y) * 1.0;
bool vignette = bool(param1.z);

vec2 mouse = vec2(Rotation, pLeft);
float globalltime = adsk_time*.05;

float time = float(param1.y) * 0.01;
...
vec3 raymarch( in vec3 ro, in vec3 rd )
{
    ...
    // dithering
    // t += 0.05*texture2DRect( iChannel0, res * gl_FragCoord.xy/iChannelResolution[0].x ).x;
