...
uniform vec3 param7;
vec2 paramPos = vec2(param6.z, param7.x) * 0.01;
float paramSpeed = float(param6.x) * 0.01;
bool paramProcedural = bool(param7.y);
uniform vec3 param3;
int steps = int(param3.y);

uniform vec3 param4;
float Detail = float(param4.x) * 0.005;
float Density = float(param3.z) * 0.005;
float Volume = float(param4.y) * 0.005;

// Added for Action camera 
uniform vec3 param8;
vec3 camera_position = vec3(param7.z, param8.x, param8.y) * 0.01;
uniform vec3 param9;
vec3 camera_interest = vec3(param8.z, param9.x, param9.y) * 0.01;
uniform vec3 param2;
float camera_roll = float(param2.z) * 0.01;
uniform vec3 param5;
float camera_fov = float(param5.z) * 0.01;
bool camera_use = bool(param3.x);
