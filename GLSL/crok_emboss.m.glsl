...
uniform vec3 param3;
vec2 position = vec2(param3.y, param3.z) * 0.01;
uniform vec3 param1;
float fLightHeight = float(param1.z) * 0.01;
uniform vec3 param2;
float fViewHeight = float(param2.x) * 0.01;
float fNormalScale = float(param1.x) * 0.01;
float spec = float(param1.y) * 0.01;
bool SHOW_NORMAL_MAP = bool(param3.x);
...
C_Sample SampleMaterial(const in vec2 vUV, sampler2D sampler,  const in vec2 vTextureSize, const in float fNormalScale)
{
    ...
    vec3 cSampleNegXNegY = texture2DRect(sampler, res * vUV + (vec2(-1.0, -1.0)) * vInvTextureSize.xy).rgb;
    vec3 cSampleZerXNegY = texture2DRect(sampler, res * vUV + (vec2( 0.0, -1.0)) * vInvTextureSize.xy).rgb;
    vec3 cSamplePosXNegY = texture2DRect(sampler, res * vUV + (vec2( 1.0, -1.0)) * vInvTextureSize.xy).rgb;
    
    vec3 cSampleNegXZerY = texture2DRect(sampler, res * vUV + (vec2(-1.0, 0.0)) * vInvTextureSize.xy).rgb;
    vec3 cSampleZerXZerY = texture2DRect(sampler, res * vUV + (vec2( 0.0, 0.0)) * vInvTextureSize.xy).rgb;
    vec3 cSamplePosXZerY = texture2DRect(sampler, res * vUV + (vec2( 1.0, 0.0)) * vInvTextureSize.xy).rgb;
    
    vec3 cSampleNegXPosY = texture2DRect(sampler, res * vUV + (vec2(-1.0,  1.0)) * vInvTextureSize.xy).rgb;
    vec3 cSampleZerXPosY = texture2DRect(sampler, res * vUV + (vec2( 0.0,  1.0)) * vInvTextureSize.xy).rgb;
    vec3 cSamplePosXPosY = texture2DRect(sampler, res * vUV + (vec2( 1.0,  1.0)) * vInvTextureSize.xy).rgb;
...
{    
    ...
    materialSample = SampleMaterial( uv, image1, resolution.xy, fNormalScale );
