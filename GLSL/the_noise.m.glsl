...
float adsk_time = float(param15.z) * 1.0;
uniform vec3 param16;
float adsk_result_frameratio = float(param16.x) * 1.0;

// global uniforms
uniform vec3 param3;
float speed = float(param3.x) * 0.01;
float offset = float(param3.y) * 0.01;
uniform vec3 param1;
float scale = float(param1.y) * 0.01;
float aspect = float(param1.z) * 0.01;
uniform vec3 param2;
float rot = float(param2.z) * 0.1;
vec2 pos = vec2(param16.y, param16.z) * 0.01;
int noise_type = int(param1.x);
...
// concrete uniforms
float c_detail = float(param3.z) * 0.01;
uniform vec3 param4;
int c_noise_itt = int(param4.x);

// fractal noise uniforms
float f_detail = float(param4.y) * 0.001;

// value noise uniforms
int v_noise_type = int(param4.z);

// worley uniform
uniform vec3 param5;
float w_detail = float(param5.y) * 0.001;
int w_color_type = int(param5.x);

// ridged noise uniforms
int r_noise_type = int(param5.z);
uniform vec3 param6;
float r_detail = float(param6.x) * 0.001;

// perlin v1 uniforms
int p1_itt = int(param6.y);
float perlinv1_v = float(param6.z) * 0.01;

// Plasma uniforms
uniform vec3 param7;
int plasma_iter = int(param7.y);
float plasma_detail = float(param7.x) * 0.01;

// Marble uniforms
uniform vec3 param8;
int marble_iter = int(param8.x);
float marble_detail = float(param7.z) * 0.01;

// Wood uniforms
int wood_iter = int(param8.z);
float wood_detail = float(param8.y) * 0.01;

// Clouds uniforms
uniform vec3 param9;
int cloud_iter = int(param9.y);
float cloud_detail = float(param9.x) * 0.01;

// Crawling uniforms
uniform vec3 param10;
int crawling_iter = int(param10.x);
float crawling_detail = float(param9.z) * 0.01;
float crawling_displace = float(param10.y) * 0.01;

// Cells on Fire uniforms
uniform vec3 param17;
float cof_detail = float(param17.x) * 1.0;
int cof_noise_type = int(param10.z);

// Water uniforms
uniform vec3 param11;
float water_detail = float(param11.x) * 0.01;
float w_noise = float(param11.y) * 0.01;

// Malone uniforms
uniform vec3 param12;
float malone_size = float(param12.y) * 0.01;
float malone_swirl = float(param12.x) * 0.01;
int malone_detail = int(param11.z);

// Slabrie uniforms
int slabrie_detail = int(param12.z);
uniform vec3 param13;
float slabrie_amp = float(param13.x) * 0.01;

// Frédéric uniforms
float fred_detail = float(param13.z) * 0.01;
int fred_itt = int(param13.y);
uniform vec3 param14;
float fred_density = float(param14.x) * 0.01;

// Flow Noise uniforms
float flow_detail = float(param14.y) * 0.01;
float flow_amp = float(param14.z) * 0.01;

// Francis Noise
int francis_detail = int(param15.x);
float francis_amp = float(param15.y) * 0.01;
