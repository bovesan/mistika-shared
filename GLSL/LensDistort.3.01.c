...
// Written by Bengt Ove Sannes (c) 2013-2014
...
void main()
{
	...
	float samples = param1.x;
    vec4 tp = vec4(0.0);
    vec2 p = gl_TexCoord[0].xy / res;
    p = (p - 0.5)*2;
    // these factors range from 0 to 0.1 for a wide-angle lens
    float axy = param2.x * 0.001;
    float ax = axy + param2.y * 0.001;
    float ay = axy + param2.z * 0.001;
    ...
    vec2 oCoord = (p/vec2(1.0-ax*xx*xx,1.0-ay*yy*yy)+1.0)*0.5;
	vec2 xy = oCoord*res;
    if (samples < 1) {
        tp = texture2DRect(image1, xy);
	} else if (samples < 2) {
		...
		for(int i=0;i<2;i++)
		{
		  tp += texture2DRect(image1, xy+jitter[i]) * m;
		}
	} else if (samples < 3) {
